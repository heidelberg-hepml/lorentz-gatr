...
class TopReconstructionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, mW_mean=80.4, mW_std=5., mt_mean=173., mt_std=20.,
        ...
        while True:
            ...
            mq = torch.randn(3, DATASET_SIZE_TEST) * mq_std + 0.
            mq[mq<=0] = -mq[mq<=0]
            mb, mq1, mq2 = mq
            ...
            pt, pW, pb, pq1, pq2 = pt[mask,:], pW[mask,:], pb[mask,:], pq1[mask,:], pq2[mask,:]
            mt, mW, mb, mq1, mq2 = mt[mask], mW[mask], mb[mask], mq1[mask], mq2[mask]

            print(f"Obtained {mask.sum(dim=0)}/{DATASET_SIZE} valid events using safety_factor={safety_factor}")
