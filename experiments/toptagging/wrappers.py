...
class TopTaggingPretrainGATrWrapper(nn.Module):
    ...
    def forward(self, batch):
        ...
        for i in range(batch.shape[0]):
            ...
            is_global = torch.zeros((x.shape[0], 1), dtype=torch.bool, device=batch.device)
            is_global[0, 0] = True
        ...
        multivector = torch.cat(multivector_list).unsqueeze(0).to(torch.device("cuda"))
        scalars = torch.cat(scalar_list).unsqueeze(0).to(torch.device("cuda"))
        is_global = torch.cat(is_global_list).to(torch.device("cuda"))
        attention_mask_list = torch.cat(attention_mask_list).to(torch.device("cuda"))
    ...
    def extract_from_ga(self, multivector, scalars, attention_mask_list, is_global):
        ...
        if self.mean_aggregation:
            ...
            batchsize = max(attention_mask_list) + 1
            logits = torch.zeros(batchsize, device=outputs.device, dtype=outputs.dtype)
            logits.index_add_(0, attention_mask_list, outputs)  # sum
            logits = logits / torch.bincount(attention_mask_list)  # mean
        else:
            logits = outputs.unsqueeze(-1)[is_global]
