...
def jetmomenta_to_precisesiast(jetmomenta, pt_min):
    ...
    precisesiast = torch.stack((x_pt, x_phi, eta, x_mass), dim=-1)
    assert torch.isfinite(
        precisesiast
...
def velocities_jetmomenta_to_precisesiast(v_jetmomenta, jetmomenta, precisesiast):
    ...
    v_x_pt = v_pt / pt
    v_x_phi = (
        -1 / (1 - phi**2 / math.pi**2).clamp(min=math.pi * EPS2) * v_phi / math.pi
    )
    v_x_mass = v_mass / mass

    v_precisesiast = torch.stack((v_x_pt, v_x_phi, v_eta, v_x_mass), dim=-1)
    assert torch.isfinite(
        v_precisesiast
...
def get_eta(particle):
    ...
    eta = stable_arctanh(particle[..., 3] / p_abs, eps=1e-10)
    return eta
