...
class PGATrCFM(EventCFM):
    """
    PGATr velocity network
    """
    ...
    def embed_into_ga(self, x, t, ijet):
        ...
        process_token = get_process_token(x, ijet, self.process_token_channels)
        t_embedding = self.t_embedding(t).expand(x.shape[0], x.shape[1], -1)
        swt = torch.cat([type_token, process_token, t_embedding], dim=-1).float()
        s = torch.cat([type_token, process_token], dim=-1).float()

        # mv embedding
        batch_shape = x.shape[:-1]
        mv = torch.zeros(
            *batch_shape, 16, dtype=x.dtype, device=x.device
        )
    ...
    def extract_from_ga(self, mv, s):
        v = torch.cat((mv[..., 0].unsqueeze(dim=-1), mv[..., 2:5]), dim=-1).squeeze(dim=-2)
        return v, s
