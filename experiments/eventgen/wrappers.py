...
def get_type_token(x_ref, type_token_channels):
    ...
    type_token = type_token.unsqueeze(0).expand(x_ref.shape[0], x_ref.shape[1], type_token_channels)
    return type_token
...
def get_process_token(x_ref, ijet, process_token_channels):
    ...
    process_token = nn.functional.one_hot(process_token_raw, num_classes=process_token_channels).squeeze()
    process_token = process_token.unsqueeze(0).expand(x_ref.shape[1], process_token_channels)
    process_token = process_token.unsqueeze(0).expand(x_ref.shape[0], x_ref.shape[1], process_token_channels)
    return process_token

class GaussianFourierProjection(nn.Module):

    def __init__(self, embed_dim, input_dim=1, scale=30.):
        super().__init__()
        # Randomly sample weights during initialization. These weights are fixed
        # during optimization and are not trainable.
        self.weights = nn.Parameter(scale * torch.randn(input_dim, embed_dim // 2), requires_grad=False)
...
class ttbarTransformerWrapper(nn.Module):
    def __init__(self, net, embed_t_dim, embed_t_scale, type_token_channels, process_token_channels):
        super().__init__()
        self.net = net
        self.type_token_channels = type_token_channels
        self.process_token_channels = process_token_channels
        self.t_embedding = nn.Sequential(GaussianFourierProjection(embed_dim=embed_t_dim, scale=embed_t_scale),
