...
def sample_uniform_in_circle(n, min_radius=0.0, max_radius=1.0):
    ...
    assert (
        0.0 <= min_radius < max_radius
    ), "Inconsistent inputs to sample_uniform_in_circle"
    ...
    while samples is None or np.sum(mask) > 0:
        ...
        mask = np.logical_or((r2 < min_radius**2), (r2 > max_radius**2))[
            :, np.newaxis
        ]
...
@gatr_cache
@torch.no_grad()
def make_full_edge_index(
    num_nodes, batchsize=1, self_loops=False, device=torch.device("cpu")
):
    ...
    if batchsize > 1:
        edge_index_list = [
            edge_index_per_batch + k * num_nodes for k in range(batchsize)
        ]
