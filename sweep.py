...
def sweep(cfg):
    ...
    Path(cfg.sweep.optuna_db).parent.mkdir(exist_ok=True)
    ...
    # Let's go
    study.optimize(
        lambda trial: run_trial(trial, seed=cfg.sweep.seed), n_trials=cfg.sweep.trials
    )
